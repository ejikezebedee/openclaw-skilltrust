...
- Score: `23`
- Policy: `baseline`
- Assurance level: `quarantine-required`
- Scanned files: `2`
...
- `review` `credentials` Capability 'credentials' requires human approval under policy 'baseline'.
- `review` `deployment` Capability 'deployment' requires human approval under policy 'baseline'.
- `review` `shell` Capability 'shell' requires human approval under policy 'baseline'.
...
- `review` SOC 2 - CC6.1 logical access controls: policy controls: POL-002, RUN-001, ADV-001
- `fail` ISO/IEC 42001 - AI system impact and risk treatment: control/probe evidence: POL-002, RUN-001, ADV-001, PROBE-001
...
- `review` EU AI Act readiness - Risk-management evidence and human oversight: human oversight gates: POL-002, RUN-001, ADV-001

## Quarantine Readiness

- `95/100` `remediation-ready` Quarantine remediation readiness
...
- quarantine verdict caps readiness at 95 until risky source is fixed
